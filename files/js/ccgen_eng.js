...
function darkside(p1,tr){
	...
	if(p1!="" && p1!=null){
		...
		for(var k=1;k<=ccghm;k++){
			...
			var cn=chkCard(p);
			for(var i=tr;i>=1;i--){
				document.console.output2.value="...انتظر من فضلك";
			...
			}else var out="Bin was wrong , Please use this formatting 123456xxxxxxxxxx  ";
		}
		if(jou==2) var out=out+"</xml>";
		else if(jou==3) var out=out+"}";
...
function cleanText(){
	var confirmar=confirm("Confirm Delete History!");
